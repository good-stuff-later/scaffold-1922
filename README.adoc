...
Elect leader among distributed nodes using a common database.
...
- Easy to reason about. Means we can express with great certainty what it will be good at
...
There may be network partitions, unexpected delays and all sorts of other problems. In practice it is 
impossible to fully guarantee the outcome of such a process.

DbLeaderElection takes a well-proven approach by using the locking mechanism of a database to do the
hard work for us. Databases like PostgreSQL, MySQL and others have battle-tested functionality for locking.
Rather than inventing our own, we use database's locking feature. Indeed we use the `SELECT .. FOR UPDATE`
statement which exists in just about any ANSI SQL compliant database. 

Also, DbLeaderElect uses a lease-based locking mechanism. The holder of the lease is the leader.
...
Thirdly, agreeing on someting in a distributed environment often means that first the nodes
must agree on what time it is. Without clocks being syncronized it will often be impossible to
agree on anything, or even worse: the algorithm may apoint more than one leader concurrently. 
DbLeaderElect avoids clock synchronization issues by solely using the time of the central database. 
It is irrelevant if the clock of the individual nodes are off.
...
may be gaps where there is no leader at all. Even long gaps, up until the interval by which the
database is contacted. If this is 30 seconds then theoretically there can at worst case 
be no leader for 29.9 seconds even when there are suitable candidates.
